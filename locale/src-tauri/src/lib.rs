...
const DEFAULT_GRAMMAR_MODEL: &str = "gemma3:4b";
const SUPPORTED_GRAMMAR_MODELS: [&str; 4] = ["gemma3:1b", "gemma3:4b", "gemma3:12b", "gemma3:27b"];
...
fn resolve_grammar_model(model: Option<String>) -> Result<String, String> {
    let requested_model = model
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or(DEFAULT_GRAMMAR_MODEL);

    if SUPPORTED_GRAMMAR_MODELS.contains(&requested_model) {
        Ok(requested_model.to_string())
    } else {
        Err(format!(
            "Unsupported grammar model '{}'. Supported models: {}",
            requested_model,
            SUPPORTED_GRAMMAR_MODELS.join(", ")
        ))
    }
}
...
fn build_grammar_prompt(language: &str, text: &str) -> String {
    let lang_name = get_language_name(language);
    format!(
        "You are a professional {} grammar and spelling corrector. Fix all grammar, spelling, and punctuation errors in the following text. Preserve the original meaning and tone. Output only the corrected text, without any explanations or commentary.\n\n{}",
        lang_name, text
    )
}
...
async fn correct_grammar(
    ...
    let selected_model = resolve_grammar_model(model)?;
    let prompt = build_grammar_prompt(&language, &text);
...
#[tauri::command]
async fn check_grammar_model_status(model: Option<String>) -> Result<String, String> {
    let selected_model = resolve_grammar_model(model)?;
    let ollama_url = get_ollama_url();
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(10))
